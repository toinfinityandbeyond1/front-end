...
export default function Research() {
  ...
  const handleReverseEngineer = async () => {
    ...
    for (let group of orGroups) {
      ...
      for (let cond of group) {
        ...
        const colType =
          columns.find((c) => c.column_name === cond.column)?.data_type || "text";
        const val = numericTypes.some((t) => colType.includes(t))
          ? Number(cond.value)
          : cond.value;

        switch (cond.operator) {
          case ">":
            query = query.gt(cond.column, val);
            break;
          case "<":
            query = query.lt(cond.column, val);
            break;
          case "=":
            query = query.eq(cond.column, val);
            break;
          default:
            break;
        }
    ...
    // Compute excluded games
    const excludedGames = allGames.filter(
      (g) => !includedGames.some((i) => i.id === g.id)
    );
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: "1rem" }}>
        {conditions.map((cond, idx) => {
          const colType =
            columns.find((c) => c.column_name === cond.column)?.data_type || "text";
          const operators =
            colType.includes("numeric") || colType.includes("integer")
              ? [">", "<", "="]
              : ["=", "contains"];
          return (
            <div
      ...
      <table style={{ borderCollapse: "collapse", width: "100%" }}>
        <thead>
          <tr>
            ...
            {columns.map((c) => (
              <th key={c.column_name} style={{ border: "1px solid #ccc", padding: "0.5rem" }}>
                {c.column_name}
              </th>
            ))}
          </tr>
      ...
      <ul>
        {Object.entries(commonFactors).map(([k, v]) => (
          <li key={k}>
            {k}: {v}
          </li>
        ))}
      ...
      <ul>
        {Object.entries(excludedFactors).map(([k, v]) => (
          <li key={k}>
            {k}: {v}
          </li>
        ))}
